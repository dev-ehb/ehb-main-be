...
export class UsersService {
  ...
  /**
   * Search EHB users for the backoffice "Assign Franchise" picker.
   * Case-insensitive matches against email + full_name. Capped at 20 rows so
   * the picker stays snappy. Returns the minimal shape backoffice needs
   * (ehb_user_id + email + full_name) - never includes password or PII.
   */
  async search(query: string, limit = 20): Promise<UserDocument[]> {
    const q = (query ?? '').trim();
    if (!q) return [];
    const safe = q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const rx = new RegExp(safe, 'i');
    return this.userModel
      .find({ is_active: true, $or: [{ email: rx }, { full_name: rx }] })
      .limit(limit)
      .exec();
  }
